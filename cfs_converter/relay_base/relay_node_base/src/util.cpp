...
unsigned int serialize_unsigned_long(unsigned long number, unsigned char *buffer) {
  unsigned int cast_value = static_cast<unsigned int>(number);
  if (cast_value != number) {
    ROS_WARN("Value exceeding the maximum value of uint64 was input, so the value changed(%lu -> %lu).", number, cast_value);
  }
  return serialize_unsigned_int(cast_value, buffer);
}
...
unsigned int serialize_long(long number, unsigned char *buffer) {
  int cast_value = static_cast<int>(number);
  if (cast_value != number) {
    ROS_WARN("Value exceeding the maximum value of uint64 was input, so the value changed(%ld -> %d).", number, cast_value);
  }
  return serialize_int(cast_value, buffer);
  // return sizeof(long);
}
...
unsigned int serialize_string(const std::string &str, unsigned char *buffer) {
  // When the receiving of strings, it may come without initialization (header.frame Id etc.)
  // If an uninitialized string type arrives,
  // the data stored in the uninitialized area will be returned when trying to get the number of bytes by length
  // Redefine it once as a character string type, and have return the number of bytes etc. firmly
