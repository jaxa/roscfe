...
**
*/
#include "cfs_serializer/ardrone_autonomy_navdata_serializer.h"
...
int serialize_ardrone_autonomy_navdata(const ::ardrone_autonomy::Navdata &send_data, unsigned char *buffer) {
  ...
  std::vector<unsigned int> tags_type;
  ...
  std::vector<unsigned int> tags_xc;
  ...
  std::vector<unsigned int> tags_yc;
  ...
  std::vector<unsigned int> tags_width;
  ...
  std::vector<unsigned int> tags_height;
...
ardrone_autonomy::Navdata deserialize_ardrone_autonomy_navdata(const unsigned char *buffer, unsigned int &length) {
  ...
  std::vector<unsigned int> tags_type = deserialize_array_unsigned_int(buffer + length, len);
  for (ii = 0; ii < tags_type.size(); ii++) {
    msg.tags_type.push_back(tags_type[ii]);
  }
  length += len;

  std::vector<unsigned int> tags_xc = deserialize_array_unsigned_int(buffer + length, len);
  for (ii = 0; ii < tags_xc.size(); ii++) {
    msg.tags_xc.push_back(tags_xc[ii]);
  }
  length += len;

  std::vector<unsigned int> tags_yc = deserialize_array_unsigned_int(buffer + length, len);
  for (ii = 0; ii < tags_yc.size(); ii++) {
    msg.tags_yc.push_back(tags_yc[ii]);
  }
  length += len;

  std::vector<unsigned int> tags_width = deserialize_array_unsigned_int(buffer + length, len);
  for (ii = 0; ii < tags_width.size(); ii++) {
    msg.tags_width.push_back(tags_width[ii]);
  }
  length += len;

  std::vector<unsigned int> tags_height = deserialize_array_unsigned_int(buffer + length, len);
  for (ii = 0; ii < tags_height.size(); ii++) {
    msg.tags_height.push_back(tags_height[ii]);
  }
  length += len;

  std::vector<float> tags_orientation = deserialize_array_float(buffer + length, len);
  for (ii = 0; ii < tags_orientation.size(); ii++) {
    msg.tags_orientation.push_back(tags_orientation[ii]);
  }
  length += len;

  std::vector<float> tags_distance = deserialize_array_float(buffer + length, len);
