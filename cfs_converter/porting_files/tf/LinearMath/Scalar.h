...
#define TF_DECLARE_ALIGNED_ALLOCATOR()                                         \
  TFSIMD_FORCE_INLINE void *operator new(size_t sizeInBytes) {                 \
    return tfAlignedAlloc(sizeInBytes, 16);                                    \
  }                                                                            \
  ...
  TFSIMD_FORCE_INLINE void operator delete[](void *ptr) {                      \
    tfAlignedFree(ptr);                                                        \
  }                                                                            \
  TFSIMD_FORCE_INLINE void *operator new[](size_t, void *ptr) { return ptr; }  \
  TFSIMD_FORCE_INLINE void operator delete[](void *, void *) {}
...
#define tfRecipSqrt(x)                                                         \
  ((tfScalar)(tfScalar(1.0) / tfSqrt(tfScalar(x)))) /* reciprocal square root  \
                                                       */
...
TFSIMD_FORCE_INLINE unsigned tfSelect(unsigned condition,
  ...
  // Set testNz to 0xFFFFFFFF if condition is nonzero, 0x000if condition is zero
...
TFSIMD_FORCE_INLINE float tfSelect(unsigned condition,
  ...
  return (float)tfFsel((tfScalar)condition - tfScalar(1.0f),
...
struct tfTypedObject {
  tfTypedObject(int objectType) : m_objectType(objectType) {}
  int m_objectType;
