...
public:
  ...
  //   void setEulerZYX(const tfScalar& yaw, const tfScalar& pitch, const
  //   tfScalar& roll) __attribute__((deprecated))
  //         {
  //           setRPY(roll, pitch, yaw);
  //         }
  //         TFSIMD_FORCE_INLINE     Quaternion& operator+=(const Quaternion& q)
  //         {
  //                 m_floats[0] += q.x(); m_floats[1] += q.y(); m_floats[2] +=
  //                 q.z(); m_floats[3] += q.m_floats[3];
  //                 return *this;
  //         }
  ...
  tfScalar length2() const { return dot(*this); }
  ...
  //         {
  //                 return Quaternion(x() * s, y() * s, z() * s, m_floats[3] *
  //                 s);
  //         }
  ...
  //         {
  //                 tfScalar s_squared = tfScalar(1.) - tfPow(m_floats[3],
  //                 tfScalar(2.));
  //                 if (s_squared < tfScalar(10.) * TFSIMD_EPSILON) //Check for
  //                 divide by zero
  //                         return Vector3(1.0, 0.0, 0.0);  // Arbitrary
  //                 tfScalar s = tfSqrt(s_squared);
  //                 return Vector3(m_floats[0] / s, m_floats[1] / s,
  //                 m_floats[2] / s);
  //         }
  ...
  //                 return Quaternion(q1.x() + q2.x(), q1.y() + q2.y(), q1.z()
  //                 + q2.z(), q1.m_floats[3] + q2.m_floats[3]);
  //         }
  ...
  //                 return Quaternion( - q2.x(), - q2.y(),  - q2.z(),  -
  //                 q2.m_floats[3]);
  //         }
  //         TFSIMD_FORCE_INLINE Quaternion farthest( const Quaternion& qd)
  //         const
  //         {
  //                 Quaternion diff,sum;
  //                 diff = *this - qd;
  ...
  //                         if (dot(q) < 0) // Take care of long angle case see
  //                         http://en.wikipedia.org/wiki/Slerp
  //                           return Quaternion((m_floats[0] * s0 + -q.x() *
  //                           s1) * d,
  //                                               (m_floats[1] * s0 + -q.y() *
  //                                               s1) * d,
  //                                               (m_floats[2] * s0 + -q.z() *
  ...
  //                                               -q.m_floats[3] * s1) * d);
  //                         else
  //                           return Quaternion((m_floats[0] * s0 + q.x() * s1)
  //                           * d,
  //                                               (m_floats[1] * s0 + q.y() *
  //                                               s1) * d,
  //                                               (m_floats[2] * s0 + q.z() *
  ...
  //                 identityQuat(tfScalar(0.),tfScalar(0.),tfScalar(0.),tfScalar(1.));
  //                 return identityQuat;
  //         }

  //         TFSIMD_FORCE_INLINE const tfScalar& getW() const { return
  //         m_floats[3]; }
};
...
// Gems 2.10. make sure v0,v1 are normalized
// {
...
//                 return Quaternion(n.x(),n.y(),n.z(),0.0f); // just pick any
//                 vector that is orthogonal to v0
//         }
